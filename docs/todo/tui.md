...
- [ ] `TUI-09` 完善消息提交失败后的恢复操作。保留未提交内容，并提供明确的重试或恢复
  路径，避免界面永久停留在 `FAILED` 且只能依赖用户猜测 `Ctrl+C` 撤回；设计时明确重试
  是否可能重复提交以及如何向用户展示结果。
  当前暂缓：`UserInputPlugin.submit()` 在任务已经进入 Runtime 队列后，仍可能因
  `InputQueuedEvent` 发布失败而向 TUI 抛错。TUI 无法区分“确定未接收”和“已接收但确认失败”，
  因此不能安全自动重试。待 Agent 层明确客户端 `request_id`、幂等接收或提交结果查询契约后
  再继续；现阶段保留队首并允许通过 `Ctrl+C` 恢复到 Composer。
...
- [x] `TUI-17` 缩短命令到首帧可见的时间。打开页面时只加载轻量 Textual 壳层，不创建
  Runtime，也不展示虚假的初始化或 Ready 状态；页面挂载后立即在后台导入、构造并启动
  `AgentRuntimeService`。首次有效提交时，若 Runtime 已 Ready 则正常发送；若尚在初始化则显示
...
- `TUI-06` 必须使用 `AgentRuntimeService` 的正式任务控制接口，不能直接访问 AgentPlugin、
