...
_RESERVED_UPLOAD_PATHS: set[Path] = set()
...
def _unique_raw_path(raw_dir: Path, filename: str) -> Path:
    ...
    if not candidate.exists() and candidate not in _RESERVED_UPLOAD_PATHS:
        return candidate
...
def _reserve_upload_path(raw_dir: Path, upload: UploadFile) -> tuple[Path, Path, str]:
    """Validate an upload's name/type and reserve a unique raw path for it.

    Synchronous and fast: allocates a unique final path in the in-process
    reservation set, while the incoming bytes go to a hidden staging path that
    the raw-directory watcher ignores. The caller holds the per-KB async lock
    while reserving, so concurrent requests cannot claim the same final path.
    """
    ...
    _RESERVED_UPLOAD_PATHS.add(saved_path)
    staging_dir = raw_dir.parent / ".openkb" / "upload-staging"
    staging_dir.mkdir(parents=True, exist_ok=True)
    try:
        fd, temp_name = tempfile.mkstemp(
            prefix=f".{saved_path.name}.", suffix=".upload", dir=staging_dir
        )
    except BaseException:
        _RESERVED_UPLOAD_PATHS.discard(saved_path)
        raise
    os.close(fd)
    return Path(temp_name), saved_path, original_name
...
async def _write_upload(
    ...
    bytes written; unlinks the hidden staging file on failure.
    """
    try:
        file_bytes = 0
...
def _reserve_add_uploads(
    ...
    reserved: list[tuple[Path, Path, str]] = []
    try:
        for upload in files:
            reserved.append(_reserve_upload_path(raw_dir, upload))
    except Exception:
        for temp_path, saved_path, _ in reserved:
            temp_path.unlink(missing_ok=True)
            _RESERVED_UPLOAD_PATHS.discard(saved_path)
        raise
    return reserved
...
async def _write_add_uploads(
    ...
    request_bytes = 0
    try:
        for (temp_path, saved_path, _original), upload in zip(reserved, files):
            request_bytes += await _write_upload(temp_path, upload, request_bytes)
        ...
        for temp_path, saved_path, _ in reserved:
            ...
            _RESERVED_UPLOAD_PATHS.discard(saved_path)
        raise
    return [(saved_path, original) for _, saved_path, original in reserved]
    ...
    try:
        while True:
            if await request.is_disconnected():
                return
            for ev in registry.events_snapshot(state):
                if ev["seq"] < next_seq:
                    continue
                next_seq = ev["seq"] + 1
