...
def _fetch_cloud_pages(col, doc_id: str) -> list[dict[str, Any]]:
    ...
    while start <= _CLOUD_PAGE_MAX:
        raw_window = col.get_page_content(
            doc_id, f"{start}-{start + _CLOUD_PAGE_WINDOW - 1}"
        )
        window = _normalize_page_content(raw_window)
        pages.extend(window)
        if not isinstance(raw_window, list) or len(raw_window) < _CLOUD_PAGE_WINDOW:
            break
