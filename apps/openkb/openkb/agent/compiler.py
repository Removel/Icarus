...
def _safe_llm_kwargs(kwargs: dict) -> dict:
    """Return diagnostic keys without credential-bearing values."""

    sensitive = {"api_key", "authorization", "proxy-authorization", "x-api-key"}
    safe = {}
    for key, value in kwargs.items():
        if key == "extra_headers" and isinstance(value, dict):
            safe[key] = {header: "[REDACTED]" for header in value}
        else:
...
def _llm_call(
    ...
    **kwargs,
    ...
    if kwargs:
        logger.debug("LLM kwargs [%s]: %s", step_name, _safe_llm_kwargs(kwargs))
