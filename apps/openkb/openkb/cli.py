...
async def _run_recompile_transaction(
    kb_dir: Path,
    doc_name: str,
    operation: Callable[[], Awaitable[None]],
) -> None:
    """Rollback all compiler-owned wiki projections when recompile fails."""

    wiki_dir = kb_dir / "wiki"
    snapshot = snapshot_paths(
        kb_dir,
        [
            wiki_dir / "summaries" / f"{doc_name}.md",
            wiki_dir / "concepts",
            wiki_dir / "entities",
            wiki_dir / "index.md",
        ],
        ...
        hardlink_dirs={wiki_dir / "concepts", wiki_dir / "entities"},
    )
    try:
        await operation()
...
def query(ctx, question, save, raw):
    ...
    if save and answer:
        explore_path = save_exploration(kb_dir, question, answer)
...
def recompile(ctx, doc_name, all_docs, dry_run, yes, refresh_schema):
    ...
    for i, meta in enumerate(targets, 1):
        ...
        if _is_long_doc(meta):
            ...
            try:
                asyncio.run(
                    _run_recompile_transaction(
                        kb_dir,
                        name,
                        lambda: compiler.compile_long_doc(
                            name, summary_path, doc_id, kb_dir, model,
            ...
            try:
                asyncio.run(
                    _run_recompile_transaction(
                        kb_dir,
                        name,
                        lambda: compiler.compile_short_doc(
                            name, source_path, kb_dir, model,
                            max_concurrency=max_concurrency,
                        ),
                    )
                )
        ...
        for meta in targets:
            ...
            if not name:
                ...
                if not doc_id:
                    ...
                    start = time.time()
                    try:
                        await _run_recompile_transaction(
                            kb_dir,
                            name,
                            lambda: compiler.compile_long_doc(
                                name, summary_path, doc_id, kb_dir, model, bundle=bundle,
                                close_async_clients=False,
                            ),
                        )
                    except Exception as exc:
                        doc = {
