...
def create_app() -> FastAPI:
    ...
    chat_session_locks: dict[tuple[str, str], asyncio.Lock] = {}

    def _kb_mutation_lock(kb_dir: Path) -> asyncio.Lock:
        key = str(kb_dir.resolve())
        lock = kb_mutation_locks.get(key)
        if lock is None:
            lock = asyncio.Lock()
            kb_mutation_locks[key] = lock
        return lock

    def _chat_session_lock(kb_dir: Path, session_id: str) -> asyncio.Lock:
        key = (str(kb_dir.resolve()), session_id)
        lock = chat_session_locks.get(key)
        if lock is None:
            lock = asyncio.Lock()
    ...
    async def kb_config_patch_endpoint(
        ...
        async with _kb_mutation_lock(kb_dir):
    ...
    async def add_endpoint(
        ...
        async with _kb_mutation_lock(resolved_kb_dir):
    ...
    async def chat_endpoint(
        ...
        session_lock = _chat_session_lock(kb_dir, session.id)

        if request.stream:
            return StreamingResponse(
                _stream_chat(
                    request, kb_dir, session, fastapi_request, bundle=bundle,
