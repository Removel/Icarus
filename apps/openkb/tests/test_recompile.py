...
def test_compile_long_doc_long_lived_loop_keeps_shared_async_clients_open(tmp_path):
    wiki = tmp_path / "wiki"
    (wiki / "summaries").mkdir(parents=True)
    (wiki / "concepts").mkdir(parents=True)
    ...
    with (
        patch.object(compiler, "_llm_call", return_value="overview"),
        patch.object(compiler, "_compile_concepts", new=AsyncMock()),
        patch.object(compiler, "_close_async_llm_clients", new=AsyncMock()) as close,
    ):
        asyncio.run(
            compiler.compile_long_doc(
                "long", summary_path, "doc-1", tmp_path, "gpt-4o-mini",
                bundle=bundle, close_async_clients=False,
            )
        )
