...
def test_fetch_cloud_pages_blank_page_does_not_truncate_later_windows():
    from openkb.indexer import _fetch_cloud_pages
    ...
    def fake_get(doc_id, rng):
        ...
        if start == 1:
            return [
                {"page": page, "content": "" if page == 500 else "x"}
                for page in range(1, 1001)
            ]
        if start == 1001:
            return [{"page": 1001, "content": "last"}]
        return []
    ...
    assert pages[-1]["page"] == 1001
    assert [call.args[1] for call in col.get_page_content.call_args_list] == [
        "1-1000", "1001-2000"
    ]
