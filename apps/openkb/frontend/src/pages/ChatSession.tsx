...
export default function ChatSession() {
  ...
  // A regenerated deck/file may reuse the same stable path. Keep an open
  // panel bound to the latest occurrence rather than its stale object.
  useEffect(() => {
    setPanelArtifact((current) => {
      if (!current) return current
      const latest = viewableArtifacts.find(
        (artifact) => artifactKey(artifact) === artifactKey(current),
      )
      if (!latest || JSON.stringify(latest) === JSON.stringify(current)) return current
      return latest
    })
    // The artifact list is derived from msgs; recompute only when the list can change.
  ...
  const changeKb = (nextKb: string) => {
    if (nextKb === kbRef.current) return
    abortRef.current?.abort()
    abortRef.current = null
    sessionIdRef.current = null
    msgsSessionIdRef.current = null
    ...
    setPanelArtifact(null)
    setKb(nextKb)
    navigate("/chat/new", { state: { kbId: nextKb } })
  }
  ...
  return (
    <div className="h-full flex">
      ...
      <Sheet
        open={panel.open}
        onOpenChange={(open) => {
          if (!open) sourceRequestRef.current += 1
          setPanel((previous) => ({ ...previous, open }))
        }}
      >
