...
- **Preserve input language**: With `preserve_input_language=True`, inferred memories keep the language and script used by the input.
...
# Keep inferred memories in the input language and script
result = m.add(messages, user_id="alice", preserve_input_language=True)
