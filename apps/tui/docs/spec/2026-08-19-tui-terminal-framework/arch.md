...
- `apps/agent/docs/spec/2026-08-16-agent-runtime-service-tui/arch.md` 继续描述
  `AgentRuntimeService`、Plugin 组装和最小 REPL 的既有边界；
- 本文覆盖其中关于终端输入、输出和交互框架的 MVP 约束；
- `apps/agent/docs/spec/2026-08-18-plugin-event-flow-current-state/arch.md` 继续作为当前 Plugin Event
  主链路的事实说明；
- 实施步骤见 `apps/tui/docs/spec/2026-08-19-tui-terminal-framework/plan.md`；
- `apps/tui/docs/spec/2026-08-19-tui-terminal-framework/plan-legacy-repl.md` 只保留为标准库 MVP 的历史实施记录。

本文不改变 Agent Core、Plugin Runtime、EventBus、Blackboard 或 Hook 的职责。

持久底部输入框、Agent 运行期间编辑和 TUI 本地双端队列的下一阶段设计见
`apps/tui/docs/spec/2026-08-19-tui-persistent-input-queue/arch.md`。下一阶段已经确认迁移为 Textual
全屏应用和应用内滚动，因此有意替代本文第一阶段的原生 scrollback 交互选择；当前实施
计划见 `apps/tui/docs/spec/2026-08-19-tui-persistent-input-queue/plan.md`。
