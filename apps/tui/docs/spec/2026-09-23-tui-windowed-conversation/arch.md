...
5. 浏览历史时新输出更新数据而不强制跳底；在对话区域右上角、紧挨右侧轨道显示英文 `↓ New messages` 入口，点击回到最新并恢复跟随。入口只在新输出到达且视图不处于跟随态时出现，用户手动滚回最新内容即视为恢复跟随并撤下入口，避免入口在无新消息时出现或在已回到最新后滞留。新消息数按新增用户轮次/可见更新的用户通知语义明确计数，不把每个 token 当一条；状态不能影响队列和运行行为。
