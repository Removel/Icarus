...
> 纠偏的提交规则以 `apps/agent/docs/spec/2026-09-20-agent-run-history-steering/arch.md` 为准。Region 所有权、注册、快照和读取
> 设计仍以本文为准。

## 文档定位

本文描述 Agent Stream Event 完成后的编排架构，以及 Blackboard 从一次性 Context 聚合器演进为多 Region 当前状态板的设计。当前已经实现的运行图以 `apps/agent/docs/spec/2026-08-18-plugin-event-flow-current-state/arch.md` 为准；Region、只读 Blackboard Tool 和完整 Run History 已实现，MemoryPlugin/KnowledgePlugin 仍按本文边界继续接入。
...
Region Registry、Region Snapshot、Blackboard 只读 Tool 和完整 Run History 已经实现；MemoryPlugin/KnowledgePlugin 仍属于后续阶段。当前实现事实继续以 `apps/agent/docs/spec/2026-08-18-plugin-event-flow-current-state/arch.md` 为准。
...
Event 使用 `apps/agent/docs/spec/2026-08-15-agent-stream-event/arch.md` 定义的通用 Event 基类。
...
最近协议完整的安全前缀，并按 `apps/agent/docs/spec/2026-09-20-agent-run-history-steering/arch.md` 使用 Assistant 中断消息闭合。
...
Memory 和 Knowledge 的完整接口见 `apps/agent/docs/spec/2026-09-15-memory-knowledge-plugin/arch.md`。
